# Week 6: Kubernetes on Google Cloud - Fchas-lab1n2-lillteamet

## 🎯 Your Team Information

- **Team Name:** Fchas-lab1n2-lillteamet
- **Namespace:** `fchas-lab1n2-lillteamet`
- **App URL:** http://fchas-lab1n2-lillteamet.chas.retro87.se
- **API Endpoint:** https://chas-academy-devops-2026-3rujcz810-erkan-djafers-projects.vercel.app/api/team-status
- **API Key:** `tfk_Z3W_Tn9e1Lmncz-iOlWiiUq6mnNlpDkGbqB8LshA9QE` _(already filled in your manifests!)_
...
kubectl get secret monitor-secret -n fchas-lab1n2-lillteamet -o jsonpath='{.data.API_KEY}' | base64 -d
...
✅ **Namespace exists** - Pre-created in GKE cluster: `fchas-lab1n2-lillteamet`
...
kubectl get namespace fchas-lab1n2-lillteamet
# Expected: Active

# Check resource quota
kubectl describe quota -n fchas-lab1n2-lillteamet
...
Your Week‑7 directory already contains a full Terraform config. Key files:

```
main.tf          # provider, backend, namespace resource
variables.tf     # all user-configurable values
terraform.tfvars  # your team-specific settings (ignored by Git)
outputs.tf        # handy values after apply
modules/          # reusable k8s-app module
redis.tf, api.tf, frontend.tf, monitor.tf, ingress.tf
```

State is stored remotely in a GCS bucket (`chas-tf-state-fchas-lab1n2-lillteamet`). This
ensures locking, encryption, versioning, and sharing across teammates.

To migrate existing local state run:

```bash
terraform init -migrate-state
```

Now `terraform plan` should report no changes.

### 📦 Modules and variables
We’ve converted the repeating deployment+service pattern into a
`modules/k8s-app` module. All services – Redis, API, Frontend – are
instantiated via `module` blocks. Variables drive image names, replicas,
environment, etc.  Edit `variables.tf` or override in `terraform.tfvars` to
change behavior without touching resource code.

### 📝 Backup & disaster recovery
Your Git repo *is* the backup for configuration. The remote state bucket has
versioning enabled; you can `gsutil ls -a` to view previous state and
`gsutil cp` to restore if needed.

If the cluster or namespace is destroyed, simply re-run:

```bash
terraform init      # pulls state from GCS
terraform apply      # recreates all resources
```

Time this sequence to calculate your RTO; decide how much data (RPO) you’re
willing to lose (Redis is a cache, so it’s cheap to accept some loss).

Add the following to `.gitignore` so that state and secrets never leak:
```gitignore
.terraform/
*.tfstate
*.tfvars
...
(See the `.gitignore` file in this directory for the complete list.)
...
sed -i 's/REPLACE_WITH_YOUR_TEAM_NAME/fchas-lab1n2-lillteamet/g' *.yaml

# Option B: Manual editing (safer if you want to review)
# Open each file and replace:
# - REPLACE_WITH_YOUR_TEAM_NAME → fchas-lab1n2-lillteamet (lowercase!)
...
⚠️ **Important:** Use lowercase with hyphens (e.g., `fchas-lab1n2-lillteamet` not `Fchas-lab1n2-lillteamet`)
...
kubectl wait --for=condition=ready pod -l app=redis -n fchas-lab1n2-lillteamet --timeout=60s

# Verify Redis is running
kubectl get pods -n fchas-lab1n2-lillteamet -l app=redis
# Expected: redis-xxx-yyy (1/1 Running)

# Check Redis logs
kubectl logs -l app=redis -n fchas-lab1n2-lillteamet
...
- Other pods can reach it at `redis-service.fchas-lab1n2-lillteamet.svc.cluster.local`
...
kubectl get configmap api-config -n fchas-lab1n2-lillteamet -o yaml

# Check what's in it
kubectl describe configmap api-config -n fchas-lab1n2-lillteamet
...
kubectl wait --for=condition=ready pod -l app=api -n fchas-lab1n2-lillteamet --timeout=90s

# Check API logs (should show Redis connection)
kubectl logs -f deployment/api -n fchas-lab1n2-lillteamet
# Expected: "Connected to Redis" and "API server listening on port 3000"

# Verify 2 replicas are running
kubectl get pods -n fchas-lab1n2-lillteamet -l app=api
...
kubectl run test --rm -it --image=curlimages/curl -n fchas-lab1n2-lillteamet -- \
...
kubectl wait --for=condition=ready pod -l app=frontend -n fchas-lab1n2-lillteamet --timeout=60s

# Check frontend status
kubectl get pods -n fchas-lab1n2-lillteamet -l app=frontend
# Expected: frontend-xxx-yyy (2/2 Running)

# Check logs
kubectl logs -f deployment/frontend -n fchas-lab1n2-lillteamet
```
...
kubectl get secret monitor-secret -n fchas-lab1n2-lillteamet
...
kubectl logs -f deployment/team-monitor -n fchas-lab1n2-lillteamet
```

**What you should see in monitor logs:**
```
🔍 Performing health check for fchas-lab1n2-lillteamet...
📊 Cluster Status:
   ✅ Pods: 5/5 running
...
🌐 Testing public endpoint...
   ✅ URL: http://fchas-lab1n2-lillteamet.chas.retro87.se
   ✅ Status: 200 OK
   ✅ Response time: 45ms
...
kubectl get ingress -n fchas-lab1n2-lillteamet

# Wait for external IP to be assigned (might take 1-2 minutes)
kubectl get ingress -n fchas-lab1n2-lillteamet -w

# Expected output:
# NAME             HOSTS                           ADDRESS         PORTS
# team-dashboard   fchas-lab1n2-lillteamet.chas.retro87.se       35.228.221.56   80
```

**What's happening:**
- Ingress controller (nginx) receives your ingress definition
- Routes traffic for `fchas-lab1n2-lillteamet.chas.retro87.se` to `frontend-service:80`
...
kubectl get all -n fchas-lab1n2-lillteamet
...
curl http://fchas-lab1n2-lillteamet.chas.retro87.se

# Expected: HTML dashboard with team name

# Test the API
curl http://fchas-lab1n2-lillteamet.chas.retro87.se/api/team

# Expected: {"name":"fchas-lab1n2-lillteamet","namespace":"fchas-lab1n2-lillteamet","version":"1.0.0"}

# Increment visit counter
curl -X POST http://fchas-lab1n2-lillteamet.chas.retro87.se/api/visits
...
3. Enter: fchas-lab1n2-lillteamet.chas.retro87.se
4. Click "Delete"
5. Close the tab
...
```bash
# Exec into API pod
kubectl exec -it deployment/api -n fchas-lab1n2-lillteamet -- sh

# Inside the pod, test DNS resolution
nslookup redis-service
# Shows: redis-service.fchas-lab1n2-lillteamet.svc.cluster.local → 10.x.x.x
...
kubectl scale deployment frontend --replicas=4 -n fchas-lab1n2-lillteamet

# Watch pods being created
kubectl get pods -n fchas-lab1n2-lillteamet -w

# Scale back down
kubectl scale deployment frontend --replicas=2 -n fchas-lab1n2-lillteamet
```
...
# Edit configuration
kubectl edit configmap api-config -n fchas-lab1n2-lillteamet

# Restart API to pick up changes
kubectl rollout restart deployment/api -n fchas-lab1n2-lillteamet
```
...
kubectl get secret monitor-secret -n fchas-lab1n2-lillteamet -o yaml
...
```bash
# Check pod status
kubectl get pods -n fchas-lab1n2-lillteamet

# Describe pod (shows events - very helpful!)
kubectl describe pod POD_NAME -n fchas-lab1n2-lillteamet
...
kubectl get pods -l app=redis -n fchas-lab1n2-lillteamet
# Expected: 1/1 Running

# 2. Check if Redis service exists
kubectl get svc redis-service -n fchas-lab1n2-lillteamet
...
kubectl logs deployment/api -n fchas-lab1n2-lillteamet | grep -i redis
```
...
kubectl describe ingress -n fchas-lab1n2-lillteamet

# 2. Verify backend service exists
kubectl get svc frontend-service -n fchas-lab1n2-lillteamet
...
nslookup fchas-lab1n2-lillteamet.chas.retro87.se
```
...
kubectl logs deployment/team-monitor -n fchas-lab1n2-lillteamet --tail=100
...
kubectl get rolebinding monitor-binding -n fchas-lab1n2-lillteamet

# 4. Test API key manually
API_KEY=$(kubectl get secret monitor-secret -n fchas-lab1n2-lillteamet -o jsonpath='{.data.API_KEY}' | base64 -d)
...
kubectl top pods -n fchas-lab1n2-lillteamet

# If quota exceeded:
# 1. Reduce replicas:
kubectl scale deployment frontend --replicas=1 -n fchas-lab1n2-lillteamet
...
│  - https://fchas-lab1n2-lillteamet.chas.retro87.se                   │
└────────────────────────────────────────────────────────┘
...
kubectl get certificate -n fchas-lab1n2-lillteamet -w
...
kubectl get challenges -n fchas-lab1n2-lillteamet -w

# You'll see:
# NAME                          STATE      DOMAIN                        AGE
# team-dashboard-tls-xxx        pending    fchas-lab1n2-lillteamet.chas.retro87.se     10s
# team-dashboard-tls-xxx        valid      fchas-lab1n2-lillteamet.chas.retro87.se     45s ← Challenge passed!
```

```bash
# 4. See the HTTP solver pod in action
kubectl get pods -n fchas-lab1n2-lillteamet -l acme.cert-manager.io/http01-solver=true
...
kubectl describe certificate team-dashboard-tls -n fchas-lab1n2-lillteamet
...
kubectl get secret team-dashboard-tls -n fchas-lab1n2-lillteamet -o yaml
...
```bash
# Test HTTPS access
curl https://fchas-lab1n2-lillteamet.chas.retro87.se

# Check certificate in browser
open https://fchas-lab1n2-lillteamet.chas.retro87.se
# Click padlock → Certificate → Details
# Issued by: Let's Encrypt
# Valid until: 90 days from now

# Test API over HTTPS
curl https://fchas-lab1n2-lillteamet.chas.retro87.se/api/team

# Check security headers
curl -I https://fchas-lab1n2-lillteamet.chas.retro87.se | grep -i strict
...
kubectl scale deployment api --replicas=5 -n fchas-lab1n2-lillteamet
kubectl scale deployment frontend --replicas=4 -n fchas-lab1n2-lillteamet
...
# Scale back down
kubectl scale deployment api --replicas=2 -n fchas-lab1n2-lillteamet
```
...
POD=$(kubectl get pod -l app=api -n fchas-lab1n2-lillteamet -o jsonpath='{.items[0].metadata.name}')
kubectl delete pod $POD -n fchas-lab1n2-lillteamet
...
kubectl edit deployment frontend -n fchas-lab1n2-lillteamet
# (Change image tag or environment variables)

# Watch rolling update
kubectl rollout status deployment/frontend -n fchas-lab1n2-lillteamet

# Rollback if needed
kubectl rollout undo deployment/frontend -n fchas-lab1n2-lillteamet
```
...
kubectl exec -it deployment/redis -n fchas-lab1n2-lillteamet -- redis-cli
...
kubectl logs -f deployment/api -n fchas-lab1n2-lillteamet --all-containers=true
...
kubectl port-forward svc/api-service 3000:3000 -n fchas-lab1n2-lillteamet
...
# Detailed pod info
kubectl describe pod POD_NAME -n fchas-lab1n2-lillteamet | grep -A 5 "Requests\|Limits"
```
...
kubectl delete -f . -n fchas-lab1n2-lillteamet

# Option 2: Delete by type
kubectl delete deployment,service,ingress,configmap,secret --all -n fchas-lab1n2-lillteamet

# Option 3: Keep namespace but remove specific app
kubectl delete deployment frontend api redis team-monitor -n fchas-lab1n2-lillteamet
kubectl delete service frontend-service api-service redis-service -n fchas-lab1n2-lillteamet
kubectl delete ingress team-dashboard -n fchas-lab1n2-lillteamet
kubectl delete configmap api-config monitor-config -n fchas-lab1n2-lillteamet
kubectl delete secret monitor-secret -n fchas-lab1n2-lillteamet
...
- [ ] Public URL loads the dashboard: `http://fchas-lab1n2-lillteamet.chas.retro87.se`
